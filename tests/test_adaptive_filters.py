...
def test_filter_reduces_known_noise(filter_function, arguments):
    sample_numbers = np.arange(500)

    clean_signal = np.sin(2 * np.pi * 0.01 * sample_numbers)
    reference_noise = 0.5 * np.sin(2 * np.pi * 0.10 * sample_numbers)
    mixed_signal = clean_signal + reference_noise
    ...
    error_before = np.mean((mixed_signal - clean_signal) ** 2)
